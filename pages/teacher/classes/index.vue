        ...
        >
          ...
          <template #item.actions="{ item }">
            <v-icon
              ...
              class="mr-2"
              @click="$confirm(
                'Вы действительно хотите удалить класс? Все обучающиеся находящиеся в нем будут также удалены. Также будут удалены задания созданые только для этого класса',
                () => remove(item)
              )"
            >
              mdi-delete
...
export default {
  ...
  methods: {
    ...
    async remove (schoolClass) {
      await SchoolClassesService.remove(this.$axios, schoolClass.uuid)
      this.updateTable()
    }
  }
}
